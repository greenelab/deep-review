...
| Term | Definition | Application data type or techniques that are used |
|----------------------------|-------------------------------------------------------------------------------------------|
| Neural network  (NN) | Machine-learning approach inspired by biological neurons where inputs are fed into one or more layers, producing an output layer | Neural networks have many different structures and training objectives. Depending on these details they can be used for many different types of machine learning tasks. |
| Supervised learning | Machine-learning approaches with goal of prediction of labels or outcomes | Most NNs for supervised learning have a small number of output nodes and are trained to match those output nodes to labels. |
| Unsupervised learning | Machine-learning approaches with goal of data summarization or pattern identification | Most NNs for unsupervised learning are trained to match input data and often have a relatively high number of output nodes for high-dimensional data. |
| Deep neural network | NN with multiple hidden layers. Training happens over the network based on the state of the output nodes. |  Deep architectures allow for feature construction to occur in early hidden layers alongside optimization of the overall training objective. |
| Feed-forward neural network (FFNN) | NN that does not have cycles between nodes in the same layer | Most of the publications that we reviewed had feed forward neural networks. However, some architecture like the recurrent neural network below incorporate cycles within layers. |
...
| Multi-layer perceptron (MLP) | Type of FFNN with at least one hidden layer where each deeper layer is a nonlinear function of each earlier layer | MLPs do not impose structure on problem and are frequently used when there is no natural ordering of the inputs (e.g., as with gene expression measurements). |
| Convolutional neural network (CNN) | A NN with layers in which connectivity preserves local structure. | CNNs are used for sequence or grid data---such as images or equally-spaced time points. _If the data meet the underlying assumptions_ performance is often good, and such networks can require fewer examples to train effectively because they have fewer parameters and also provide improved efficiency. |
| Long short-term memory (LSTM) neural network | This special type of RNN has features that enable models constructed via this approach to capture longer-term dependencies. | LSTMs are gaining a substantial foothold in the analysis of natural language, and may become more widely applied to biological sequence data in the future. |
...
| Variational Autoencoder (VAE) | This special type of AE has the added constraint that the model is trained to learning normally distributed features. | VAEs have a track record of producing a valuable reduced representation in the imaging domain, and some early publications have used VAEs to analyze gene expression data. |
| Denoising Autoencoder (DA) | This special type of AE includes a step where noise is added to the input during the training process. | Like AEs, DAs have been used for unsupervised analysis of gene expression data as well as data extracted from the electronic health record. The denoising step acts somewhat as smoothing and may allow for effective use on noisier input data. |
...
| Generative adversarial network (GAN) | A generative NN approach where two neural networks are trained. One neural network, the generator, is provided with a set of randomly generated inputs and tasked with generating samples. The second, the discriminator, is trained to differentiate real and generated samples. After the two neural networks are trained against each other, the resulting generator can be used to produce new examples. | GANs have been used in a number of areas. One area that has seen numerous recent publications is the use of GANs to synthesize new datasets with the same statistical properties of datasets that contain individual-level records and are subject to sharing restrictions. |
| Adversarial training | A process by which artificial training examples are maliciously designed to fool a NN and then input as training examples to make the resulting NN robust (no relation to GANs) | Adversarial training has been used in the analysis of images; however, we expect that there are numerous opportunities for adversarial training in biology and medicine to provide performance improvements though these are largely still unexplored. |
