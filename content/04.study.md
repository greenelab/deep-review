...
First, because the inputs (ChIP-seq measurements) are continuous and most current algorithms are designed to produce binary outputs (whether or not there is TF binding at a particular site), false positives or false negatives can result depending on the threshold chosen by the algorithm.
...
For many cell types, we often have experimental TF binding data for a certain cell type (e.g. normal human), but not the one of interest (e.g. cancer).
The in vivo binding landscapes of TFs vary widely across different cellular context, making cross cell-type prediction difficult. 
While the number of in vivo TF-binding datasets continue to grow, it is still not possible to perform TF ChIP-seq assays for every expressed TF against all cell types/tissues under all possible physiological conditions.
Thus, we need computational approaches to predict in a context where we don't have the data.

To solve this problem, there are several different directions.
One option is to use cell type specific features such as epigenetic marks.
If these features are available, this is likely the best approach for accurate prediction since they will be strong signals for that particular cell type and TF.
Another option is to use domain adaptation, where we used a trained model from one domain and transfer the model to predict on features in another domain.
This has been done in many other areas such as sentiment analysis [@url:http://citeseerx.ist.psu.edu/viewdoc/summary?doi=10.1.1.231.3442], where training is done on the reviews for books, and prediction is on the reviews for movies.
...
TFImpute [@tag:Qin2017_onehot] predicts binding in new cell type-TF pairs, but they require the cell types to be in the training set for other TFs.
This is a step in the right direction, but a more general domain transfer model across cell types would be more useful.


Recently, the "ENCODE-DREAM in vivo Transcription Factor Binding Site Prediction Challenge" was introduced to provide data for this type of problem, as well as determine baseline evaluations for different types of approaches. 
The goal of the challenge is to find models for predicting positional in vivo TF binding maps across cell types and tissues.
In the ENCODE-DREAM challenge, there are 4 types of data to use for cross-context prediction: (1) DNA sequence, (2) in vitro DNA shape, (3) chromatin accessibility (DNase-seq) and (4) gene expression (RNA-seq) data.
These types of data are relatively easy to extract for a certain context, providing a standard input across contexts for computational models. 

One deep learning approach on the ENCODE-DREAM challenge was FactorNet[@tag:Quang2017_factor].
In this model, they use the one-hot encoded DNA sequence matrix, along with the per-nucleotide DNase-seq data as the input to a convolutional-recurrent network.
They then append gene expression, gene annotation, and CpG island features to the dense layer of the network. 
...
However, since only a small fraction of the genome differs between human and mouse, it is difficult to implement robust transfer learning.
This was done using transfer learning of string kernels by Singh et al. [@tag:Singh2016_tsk] which showed improvements over previous methods, but has yet to be applied in deep learning models.
