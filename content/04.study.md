...
One deep learning approach on the ENCODE-DREAM challenge was FactorNet[@tag:Quang2017_factor]. In this model, they use the one-hot encoded DNA sequence matrix, along with the per-nucleotide DNase-seq data as the input to a convolutional-recurrent network. They then append gene expression, gene annotation, and CpG island features to the dense layer of the network. 
