...
Guo et al. [@tag:guo_calibration] observed that contemporary neural networks are poorly calibrated and provide a simple recommendation for calibration: temperature scaling, a single parameter special case of Platt scaling [@tag:platt_scaling].
...
Krueger et al. introduced Bayesian hypernetworks and used an invertible generative network to map isotropic Gaussian noise to parameters of the primary network.
Using an invertible hypernetwork allowed computationally cheap sampling and tractable estimation of the posterior.
