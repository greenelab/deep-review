...
Krueger et al. introduced Bayesian hypernetworks [@tag:bayesian_hypernets] as another framework for approximate Bayesian inference in deep learning, where the authors used an invertible generative hypernetwork to map isotropic Gaussian noise to parameters of the primary network and trained both the hypernet and the primary net as a single model.
